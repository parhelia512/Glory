...
namespace Glory
{
	...
	typedef bool (__cdecl* OnLoadExtraProc)(GloryContext*, const char*, Module*, Module*);
	...
	void EngineLoader::LoadExtra(const std::string& name, const std::filesystem::path& path, Module* pModule, Module* pRequiredModule)
	{
		...
		{
			FreeLibrary(lib);
			Debug::LogError("Failed to load module extra: " + name + ": Missing OnLoadExtra function!");
			return;
		}

		GloryContext* pContext = GloryContext::GetContext();
		if (!(loadProc)(pContext, dllPath.parent_path().string().data(), pModule, pRequiredModule))
		{
			FreeLibrary(lib);
			Debug::LogError("Failed to load module extra: " + name + ": OnLoadExtra return false!");
			return;
		}
