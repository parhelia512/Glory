...
namespace Glory
{
	...
	struct FSMNode
	{
		 ...
		 */
		FSMNode(const std::string& name, UUID id):
			m_Name(name), m_ID(id) {}
	...
	struct FSMProperty
	{
		/** @brief Constructor */
		FSMProperty():
			m_Name("New Property"), m_Type(FSMPropertyType::Number) {}
		/** @override
		 * @param name Name of the property
		 * @param id ID of the property
		 */
		FSMProperty(const std::string& name, FSMPropertyType type, UUID id):
	...
	public:
		...
		GLORY_FSM_API size_t NodeCount() const;
		/** @brief Get number of transitions in this state machine */
		GLORY_FSM_API size_t TransitionCount() const;
		/** @brief Get a node in this state machine
		 * @param index Index of the node to get
		 */
		GLORY_FSM_API const FSMNode& Node(size_t index) const;
		/** @overload */
		GLORY_FSM_API const FSMNode* Node(UUID id) const;
		/** @overload */
		GLORY_FSM_API FSMNode* FindNode(UUID id);
		/** @brief Get a transition in this state machine
		 * @param index Index of the transition to get
		 */
		GLORY_FSM_API const FSMTransition& Transition(size_t index) const;
		/** @overload */
		GLORY_FSM_API const FSMTransition* Transition(UUID id) const;
		/** @brief Add a new node to the state machine
		 * @param name Name of the state
		 * @param id ID of the node
		 */
		GLORY_FSM_API FSMNode& NewNode(const std::string& name, UUID id=UUID());
		 ...
		 * @param id ID of the transition
		 */
		GLORY_FSM_API FSMTransition& NewTransition(const std::string& name, UUID from, UUID to, UUID id=UUID());
		/** @brief Index of the node the state machine starts in */
		GLORY_FSM_API size_t StartNodeIndex() const;
		/** @brief ID of the node the state machine starts in */
		GLORY_FSM_API UUID StartNodeID() const;
		/** @brief Set the node this state machine starts in
		 * @param index Index of the starting node
		 */
		GLORY_FSM_API void SetStartNodeIndex(size_t index);

		/** @brief Find a state/node by name
		 * @param name Name of the state to find
		 */
		GLORY_FSM_API const FSMNode* FindNode(std::string_view name) const;
		/** @brief Find a transition by name
		 * @param name Name of the transition to find
		 */
		GLORY_FSM_API const FSMTransition* FindTransition(std::string_view name) const;
		/** @overload */
		GLORY_FSM_API FSMTransition* FindTransition(UUID transitionID);

		/** @brief Find the index of a node/state by id
		 * @param id ID of the node
		 */
		GLORY_FSM_API size_t NodeIndex(UUID id) const;
		/** @brief Find the index of a transition by id
		 * @param id ID of the transition
		 */
		GLORY_FSM_API size_t TransitionIndex(UUID id) const;
	...
	public:
		 ...
		 */
		GLORY_FSM_API void SetPropertyValue(FSMData* pFSM, std::string_view name, void* data);
		/** @brief Get the value of a property in this state machine instance
		 * @param pFSM FSM data resource
		 * @param name Name of the property
		 * @param out Value to write to
		 */
		GLORY_FSM_API void GetPropertyValue(FSMData* pFSM, std::string_view name, void* out) const;
		/** @brief Update the state machine if any properties have changed since the last frame */
		GLORY_FSM_API void Update();
