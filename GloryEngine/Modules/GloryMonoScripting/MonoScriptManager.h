...
namespace Glory
{
	...
	public:
		...
		GLORY_MONO_API void Initialize(AssemblyClass* pEngineClass, MonoObject* pEngineObject);
		/** @brief Find the index of a type by name */
		GLORY_MONO_API int TypeIndexFromName(std::string_view name) const;
		/** @brief Find the index of a type by hash */
		GLORY_MONO_API int TypeIndexFromHash(uint32_t hash) const;
		/** @brief Get a dummy object for a type index */
		GLORY_MONO_API MonoObject* Dummy(size_t index) const;
		/** @brief Get the type name of a type index */
		GLORY_MONO_API std::string_view TypeName(size_t index) const;
		/** @brief Get the type hash of a type index */
		GLORY_MONO_API uint32_t TypeHash(size_t index) const;
		/** @brief Get the number of types */
		GLORY_MONO_API size_t TypeCount() const;

		/** @brief Resize a property values array and copy the default values of the script into it */
		GLORY_MONO_API void ReadDefaults(size_t typeIndex, std::vector<char>& dest) const;

		/** @brief Get the script properties of a script type */
		GLORY_MONO_API const std::vector<ScriptProperty>& ScriptProperties(size_t typeIndex) const;

		/** @brief Write property values to MonoObject */
		GLORY_MONO_API void SetPropertyValues(size_t typeIndex, MonoObject* pMonoObject, std::vector<char>& data) const;
		/** @brief Read property values from MonoObject */
		GLORY_MONO_API void GetPropertyValues(size_t typeIndex, MonoObject* pMonoObject, std::vector<char>& data) const;
		/** @brief Invoke a method on a script */
		GLORY_MONO_API void Invoke(size_t typeIndex, MonoObject* pMonoObject, const std::string& method, void** args);
